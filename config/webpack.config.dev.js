...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      // @remove-on-eject-begin Resolve Babel runtime relative to react-scripts. It
      // usually still works on npm 3 without this but it would be unfortunate to rely
      // on, as react-scripts could be symlinked, and thus babel-runtime might not be
      // resolvable from the source.
      'babel-runtime': path.dirname(require.resolve('babel-runtime/package.json')),
      // @remove-on-eject-end Support React Native Web
  ...
  module: {
    ...
    rules: [
      ...
      // ** ADDING/UPDATING LOADERS ** The "file" loader handles all assets unless
      // explicitly excluded. The `exclude` list *must* be updated with every change
      // to loader extensions. When adding a new loader, you must add its `test` as a
      // new entry in the `exclude` list for "file" loader. "file" loader makes sure
      // those assets get served by WebpackDevServer. When you `import` an asset, you
      // get its (virtual) filename. In production, they would get copied to the
      // `build` folder.
      {
        exclude: [
          /\.html$/,
      ...
      // Process JS with Babel.
      {
        ...
        options: {
          ...
          // @remove-on-eject-end This is a feature of `babel-loader` for webpack (not
          // Babel itself). It enables caching results in
          // ./node_modules/.cache/babel-loader/ directory for faster rebuilds.
      ...
      // to the "file" loader exclusion list.
      ...
      // Parse less files and modify variables
      {
        ...
        use: [
          ...
          },
          {
            ...
            options: {
              modifyVars: { "@primary-color": "#250ee9" },
            },
          },
        ],
      },
